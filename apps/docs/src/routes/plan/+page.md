...
- **Confirmed bug/limitation, not fixed on our end:** graph creation (`CREATE GRAPH x` / lowercase `create graph x`, matching upstream's exact statement) hangs indefinitely — no response, no bytes, no server-side log line, server otherwise stays healthy. Ruled out three explanations by testing directly: not case-sensitivity (both cases hang identically), not a stuck write-lock from an earlier aborted client (still hangs on a freshly restarted container process, same data volume), not missing warmup (still hangs immediately after a successful `LIST GRAPH`). This is a real limitation of the `turingdbai/turingdb:nightly` build we're running (`977b3693b483f42a496773fee3617d68a7762c51`, 07-05-2026) — reads work fine, this specific write does not. Needs a client-side timeout (see upstream's per-call `AbortSignal.timeout()`, which our provider doesn't have yet) so this fails fast instead of hanging forever; the underlying server bug isn't ours to fix. Re-test against a newer image tag before relying on graph creation in Phase 1.
